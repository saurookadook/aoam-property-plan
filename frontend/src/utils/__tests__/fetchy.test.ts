...
import fetchy, { DEFAULT_FETCH_HEADERS } from '@/utils/fetchy';
...
function MockResponsePromise(
  bodyJson: KeyedObject | null,
  options: RequestInit,
): Promise<Response> {
...
describe("fetchy - a 'fetch' wrapper", () => {
  ...
  describe('PUT requests', () => {
    it('should make request and retrieve response (with csrfToken?)', async () => {
      const updateLuciferName = {
        id: '3c679338-c262-45ba-bc0d-208229744931',
        name: 'Fluffy',
      };
      const response = await fetchy
